...
-- Одна из основных функций. Выполняет скрипт из каталога game/lua/ указанный в аргументе, подключая его в основное окружение Lua. Так вы можете разделять функционал на разные файлы, создавая подобие ООП.
...
-- Функции установки курсора на позицию не влияют на этот вывод, поэтому (не обязательно) можно указать позицию, на которой будет отрисовано сообщение: Left — левый край консоли, Top — верхний край.
-- Её можно вызвать в любой точке кода, и сообщение всё равно будет показано, так как вывод осуществляется после очистки консоли.
...
CurL()
CurT()
-- Возвращают текущую позицию текстового курсора в консоли: Left, Top соответственно. После любого вывода (например, print()) эти значения меняются.
...
SetScene('Имя')
-- Меняет текущую сцену на указанную (без расширения .lua). Сцена изменится независимо от наличия файла в каталоге game/scenes — просто будет ошибка на экране, пока сцена там не появится.

ShutDownGame()
-- Меняет состояние игры IsRunning на false. После завершения текущего игрового цикла консоль закроется.

CreateConVar('имя', любое значение, min, max)
-- Создаёт "консольную переменную" с указанным именем и значением. Если значение — число, то можно указать лимиты. Такие переменные используются в game/cfg/config.lua для гибкой настройки ядра C#. Вы можете использовать это как глобальные переменные.
-- CreateConVar('sv_cheats', false)

SetConVar('имя', значение)
-- Устанавливает значение указанной консольной переменной. Значение может быть nil.
...
-- Создаёт сущность и возвращает её объект. Объект имеет поля и методы класса Entity. Имя может повторяться. Позицию обязательно указывать в фигурных скобках.
...
-- Клавиша горячей перезагрузки Lua-окружения.

CreateConVar('fps_target', 20, 1)
-- Частота обновления консоли.

CreateConVar('DebugChar', ' ')
-- Символ-заполнитель, который очищает консоль.
